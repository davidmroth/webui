import { json } from '@sveltejs/kit';
import { renameConversationFromHermes } from '$server/chat';
import { getConfig } from '$server/env';
import { noteHermesWorkerAuthFailure, noteHermesWorkerHeartbeat } from '$server/hermes-heartbeat';

function isAuthorized(request: Request) {
  const expected = getConfig().hermesServiceToken;
  const authHeader = request.headers.get('authorization') || '';
  return authHeader === `Bearer ${expected}`;
}

export async function POST({
  params,
  request
}: {
  params: { conversationId: string };
  request: Request;
}) {
  if (!isAuthorized(request)) {
    noteHermesWorkerAuthFailure('title');
    return json({ error: 'Unauthorized' }, { status: 401 });
  }

  noteHermesWorkerHeartbeat('title');

  const body = await request.json().catch(() => ({}));
  const title = typeof body?.title === 'string' ? body.title.trim() : '';
  if (!title) {
    return json({ error: 'Conversation title is required.' }, { status: 400 });
  }

  const result = await renameConversationFromHermes(params.conversationId, title);
  if (!result.ok) {
    return json({ error: 'Conversation not found.' }, { status: 404 });
  }

  return json({ ok: true, title: result.title });
}
