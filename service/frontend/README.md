...
7. Set `BRIEFING_RENDERER_BASE_URL` and `BRIEFING_RENDERER_SERVICE_TOKEN` if you want the archive page's regenerate action to call the briefing renderer API. In Docker on macOS, a common base URL is `http://host.docker.internal:9910` when the renderer is running on your host.
8. Set `WEB_PUSH_VAPID_SUBJECT`, `WEB_PUSH_VAPID_PRIVATE_KEY`, and `PUBLIC_WEB_PUSH_VAPID_PUBLIC_KEY` to enable background Web Push notifications. A practical way to generate the keys is `npx web-push generate-vapid-keys`.
9. Run `docker compose up --build` from this directory.
10. Open `http://localhost:3000` by default, or use whatever value you set for `WEBUI_PORT` in `.env`. If you access the dev server through a custom hostname such as `ai.local`, add it to `__VITE_ADDITIONAL_SERVER_ALLOWED_HOSTS` in `.env`. The exposed MySQL and MinIO ports are also configurable there. `MYSQL_HOST_PORT` only changes the port published to your host machine; the `webui` container still connects to the `mysql` service on internal port `3306` so host-port changes do not break container-to-container traffic. Similarly, `MINIO_API_PORT` is host-facing only; `webui` should connect to MinIO on internal port `9000`.
11. Sign in with the bootstrap key from `.env`.
