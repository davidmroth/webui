...
3. `HERMES_EVENT_LEASE_SECONDS` controls how long an in-flight Hermes event can stay in `processing` **without progress** before the web UI marks it stale. Progress is any Hermes call that renews `last_progress_at` (assistant posts/stream chunks, typing indicators). Stale cleanup runs automatically on every Hermes inbox poll, on the diagnostics monitor interval (`DIAGNOSTICS_STALE_CHECK_INTERVAL_SECONDS`, default 15s), and whenever queue or conversation status is queried. When the Hermes worker heartbeat is offline, dequeue may still reclaim stale work for retry; once the lease expires with no progress, the event is marked `stale` instead of staying in `processing` indefinitely.
