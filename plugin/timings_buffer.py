...
def decode_timings_fingerprint(raw: Any) -> Optional[dict[str, Any]]:
    """Decode proxy-stamped ``system_fingerprint`` (survives OpenAI extra=ignore)."""
    if not isinstance(raw, str) or not raw:
        return None
    idx = raw.find(_FINGERPRINT_PREFIX)
    if idx < 0:
        return None
    blob = raw[idx + len(_FINGERPRINT_PREFIX) :]
    out: dict[str, Any] = {}
...
def extract_timings_from_api_response(
    ...
    *,
    ...
    usage_kw = _to_mapping(usage) or {}
    usage_from_response = _to_mapping(response_map.get("usage")) or {}
    raw_usage = getattr(response, "usage", None) if response is not None else None
    usage_from_attr = _to_mapping(raw_usage) or {}
    usage_extra = getattr(raw_usage, "model_extra", None) if raw_usage is not None else None
    if not isinstance(usage_extra, dict):
        extra_map = getattr(response, "model_extra", None) if response is not None else None
        usage_extra = extra_map if isinstance(extra_map, dict) else {}

    # Response-side usage/timings win over the stripped Hermes hook usage dict.
    usage_map: dict[str, Any] = {}
    for part in (usage_kw, usage_from_response, usage_from_attr, usage_extra):
        if part:
